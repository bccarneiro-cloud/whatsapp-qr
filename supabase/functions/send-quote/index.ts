...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Manual auth (we deploy with --no-verify-jwt)
    const authHeader =
    ...
    // Validate token + get user (client-side key)
    ...
    if (!quote_request_id) {
      return new Response(
        JSON.stringify({ ok: false, error: "quote_request_id em falta." }),
    ...
    const { data: qr, error: qrErr } = await sbService
      .from("quote_requests")
      .select(
        "id, user_id, event_type, event_date, location, guests, details, final_price, admin_notes, status"
      )
    ...
    const { data: userData, error: userErr } =
      await sbService.auth.admin.getUserById(qr.user_id);
